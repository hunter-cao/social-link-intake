...
6. For videos, run `scripts/xhs_media_evidence.mjs` when temporary media access is available. It downloads media only to `/private/tmp` or the Windows temp directory, uses ffprobe/ffmpeg to detect audio/video streams, extracts temporary keyframes/audio evidence, and deletes the files after the run. On Windows, prefer user-level npm installer packages for ffmpeg/ffprobe when system installation is unavailable.
